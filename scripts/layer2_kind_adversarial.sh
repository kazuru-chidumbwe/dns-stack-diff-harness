#!/usr/bin/env bash
# Kind Layer-2: shared-injector adversarial pins on consecutive hops
# (NodeLocal 169.254.20.10 ↔ CoreDNS 10.96.0.10) for lab.stackdiff.
#
# Usage on Lab Test Server:
#   KUBECTL='docker exec kosv-control-plane kubectl' ./scripts/layer2_kind_adversarial.sh
#
...
OUT="$ROOT/artifacts/layer2-kind-$STAMP"
NS=stackdiff-l2
...
log() { echo "[L2 $STAMP] $*" | tee -a "$OUT/run.log"; }
...
# --- backup Corefile text only (avoids resourceVersion apply conflicts) ---
...
# Auth CoreDNS for lab.stackdiff
...
# Fix NS A to ClusterIP placeholder — auth pod answers from itself; use service DNS later
# Keep zone as-is (A records are the answers clients need).
...
# Load images into kind node containerd if needed
...
# Patch CoreDNS: lab.stackdiff → MITM (set after first mitm deploy)
patch_coredns_to_mitm() {
  local mitm_ip="$1"
  local corefile
  corefile="$($KUBECTL -n kube-system get cm coredns -o jsonpath='{.data.Corefile}')"
  # strip any prior stackdiff block
  corefile="$(printf '%s\n' "$corefile" | awk '
    BEGIN{skip=0}
...
patch_nodelocal_lab_zone() {
  # Forward lab.stackdiff to kube-dns-upstream so dig@NodeLocal traverses CoreDNS→MITM
  local nl
...
# First mitm (passthrough) to learn IP, patch DNS path once
...
for mode in additional-glue malformed-truncate; do
  mkdir -p "$OUT/modes/$mode"
  deploy_mitm "$mode"
  # ClusterIP usually stable for same Service; re-patch if changed
  NEW_IP="$($KUBECTL -n "$NS" get svc stackdiff-mitm -o jsonpath='{.spec.clusterIP}')"
  if [[ "$NEW_IP" != "$MITM_IP" ]]; then
    MITM_IP="$NEW_IP"
    patch_coredns_to_mitm "$MITM_IP"
  fi
  # bounce coredns to drop any zone cache
  $KUBECTL -n kube-system rollout restart deploy/coredns
  $KUBECTL -n kube-system rollout status deploy/coredns --timeout=180s
  sleep 3
  dig_both "$mode" pin
...
DIGS_SHA="$(sha256sum "$OUT/modes/additional-glue/digs-pin.txt" "$OUT/modes/malformed-truncate/digs-pin.txt" 2>/dev/null | sha256sum | awk '{print $1}')"

cat >"$OUT/SUMMARY.md" <<EOF
# Layer-2 Kind adversarial consecutive-hop pins ($STAMP)
...
- Combined adversarial digs fingerprint: \`$DIGS_SHA\`
...
log "DONE artifact=$OUT digs_fingerprint=$DIGS_SHA"
echo "$OUT"

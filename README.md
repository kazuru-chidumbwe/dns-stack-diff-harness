...
| Role | Tag | Notes |
| --- | --- | --- |
| **SemVer (latest)** | [`v0.1.14`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.14) | Table 9 n=3 at TTL 20s/60s plus cache-60/zone-300 mismatch on Kind and compose; lag tracks remaining cache TTL |
| **SemVer (serial-hop, matched sweep)** | [`v0.1.13`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.13) | Serial-hop TTL relationship on two pairings, 20–300s, n=1 per TTL. Long-TTL rows still cited from this tag |
| **SemVer (serial-hop, self-forward)** | [`v0.1.12`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.12) | Polled timing at TTL 20s (15.65s). NodeLocal forwarded to an address it bound; superseded by v0.1.13 |
| **SemVer (serial-hop, superseded)** | [`v0.1.11`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.11) | Real serial-hop cache-staleness experiment: NodeLocal->CoreDNS->auth with caching enabled — the paper's first genuine consecutive-hop finding. Superseded by v0.1.12's timing fix |
| **SemVer (version-window)** | [`v0.1.10`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.10) | Version-window PROMOTE re-run on Host B with image-tag/ID capture in the manifests (was asserted only in decision.json before) |
| **SemVer (Kind L2 parallel)** | [`v0.1.9`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.9) | Kind L2 parallel-topology re-run: NodeLocal and CoreDNS each forward to the MITM independently; malformed-truncate now shows a genuine divergence |
| **SemVer (oracle hardening)** | [`v0.1.8`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.8) | Oracle both-null guard, failure-class fix, ΔD value-pair hardening, image-tag manifest capture, artifact-audit fixes |
| **SemVer (version-window)** | [`v0.1.7`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.7) | Host B version-window PROMOTE instance (Unbound 1.24.0→1.25.1, MITM mode fixed) |
| **SemVer (Kind L1/L2)** | [`v0.1.6`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.6) | Kind L1/L2 evidence packs shipped into `artifacts/`; `.gitignore` allowlist fix |
| **SemVer** | [`v0.1.5`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.5) | Host B PC+matrix · null-aware (last tag with a minted Zenodo DOI; v0.1.6 onward cite the GitHub tag directly, not Zenodo) |
...
- **Host B + null-aware cite:** cite **`v0.1.5`** · pack [`artifacts/hostb-20260904T204650Z/`](artifacts/hostb-20260904T204650Z/) · Zenodo version DOI [10.5281/zenodo.22313122](https://doi.org/10.5281/zenodo.22313122) (concept [10.5281/zenodo.21959550](https://doi.org/10.5281/zenodo.21959550)); last tag with a minted DOI — `v0.1.6` onward cite the GitHub tag directly
- **Kind L1 evidence pack:** cite **`v0.1.6`** · `artifacts/layer1-kind-20260904T231736Z/`
- **Kind L2, parallel topology (load-bearing):** cite **`v0.1.9`** · `artifacts/layer2-kind-parallel-20260912T112004Z/` — NodeLocal and CoreDNS each forward to the MITM independently; passthrough/additional-glue agree, malformed-truncate genuinely diverges (NodeLocal times out, CoreDNS returns FORMERR)
...
- **Host B version-window PROMOTE instance (load-bearing):** cite **`v0.1.10`** · `artifacts/adversarial-20260912T113912Z/` (pre) · `artifacts/adversarial-20260912T113934Z/` (post) · `artifacts/change-window-version-20260912T113953Z/` (decision) · manifests carry `container_images` (tag + resolved image ID from `docker inspect`)
...
- **Oracle hardening + artifact-audit fixes:** cite **`v0.1.8`** · `all_null`/`classify_failure`/`delta_divergence` in `classifier/oracle.py`
- **Serial-hop cache-staleness (load-bearing):** cite **`v0.1.14`** · Kind packs `artifacts/layer2-kind-serial-cache-20260915T202228Z/` (and 202259Z / 202339Z / 202421Z / 202543Z / 202704Z / 202827Z) and compose packs `artifacts/layer2-compose-serial-cache-20260915T201622Z/` (and 201651Z / 201721Z / 201751Z / 201901Z / 202010Z / 202120Z) — n=3 at TTL 20s and 60s; one cache-60/zone-300 mismatch per pairing; lag tracks remaining cache TTL. Matched 120s/300s n=1 packs remain on **`v0.1.13`**.
- **Serial-hop (superseded self-forward):** `v0.1.12` · `artifacts/layer2-kind-serial-cache-20260912T181106Z/` — polled 15.65s at TTL 20s; NodeLocal bound and forwarded `10.96.0.10`
- **Serial-hop (superseded upper bound):** `v0.1.11` · `artifacts/layer2-kind-serial-cache-20260912T130932Z/` — sleep duration, not a measurement
- **Package cite (Aug 2026 + Package B/C):** cite **`v0.1.2`** (last tag with a minted Zenodo DOI, [10.5281/zenodo.21961205](https://doi.org/10.5281/zenodo.21961205), kept for historical citations only)

...
| Role | Tag | Notes |
| --- | --- | --- |
| **SemVer (latest)** | [`v0.1.12`](https://github.com/kazuru-chidumbwe/dns-stack-diff-harness/releases/tag/v0.1.12) | Fixed serial-hop cache-staleness timing: ~1s-interval polling replaces a single post-hoc sample, turning an upper bound (~30.3s) into a measurement (15.65s, cross-checked against a 15.54s TTL-based prediction) |
...
- **Serial-hop cache-staleness experiment (load-bearing):** cite **`v0.1.12`** · `artifacts/layer2-kind-serial-cache-20260912T181106Z/` — real consecutive-hop chain (NodeLocal cache -> CoreDNS -> auth); a real zone-record change is served stale by NodeLocal for a measured 15.65s (cross-checked against a 15.54s TTL-based prediction) before reconverging · `v0.1.11`'s pack reported the same chain with a flawed ~30.3s upper bound instead of a measurement, superseded

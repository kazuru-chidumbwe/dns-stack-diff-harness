...
v0 uses **pair mode**: Unbound and CoreDNS-forwarder as **role stand-ins** for path hops (local forwarder/cache ≈ NodeLocal / systemd-resolved; full recursive/validating ≈ CoreDNS recursive or upstream). Path mode (chained hops) comes after the pair-mode oracle is trusted.
...
| `dnssec_posture.unbound` / `coredns_fwd` | Per-resolver validation setting for this profile |
...
| coredns_fwd | Validation **off** unless trust anchors / `--dnssec` configured |
